...
L = 2.2 # Original L=2.2
H = 1 # Original H=0.41
c_x = 0.2 # Original c_x=0.2
c_y = 0.5 # Original c_y=0.2
r = 0.05 # Original r=0.05
...
# Creating distance field from the obstacle.
...
tol=1e-15 # I can use as tol one of those values: 1e-5; 1e-8 or even 1e-15?
tot_save= 5000 # we save every -tot_save- steps

step=0
step_init = 2000 #(step at which the calculation of u_mean, p_mean and f starts)
#stop_to_restart = 120 # step at which to stop the code and save everything!! (to then restart)
step_final = 100 #100000000 # 100 milions
...
# Let's create the directory (if it already exists, no error will be raised)
folder_check.mkdir(exist_ok=True, parents=True) 
folder_para.mkdir(exist_ok=True, parents=True)


if restart==False:
    # writing the mesh inside the .bp files that we will use to reload the values during a reset
    ...
    # Open the .bp files in write mode
...
if restart:
    ...
    # Let's create new .bp files (for Paraview) that start directly from the restart step
...
while step <= step_final:  
    ...
    if step >= step_init:
        ...
        # It could be set a more relaxed tolerance!! Such as 1e-5 or 1e-8
        ...
        if step % tot_save==0:
            MPI.COMM_WORLD.Barrier()
            # I save the solution every tot steps
            adios4dolfinx.write_function(check_file_u, u1, time=t, name="u1")
            adios4dolfinx.write_function(check_file_u_mean, u_mean, time=t, name="u_mean")
            adios4dolfinx.write_function(check_file_f, f, time=t, name="f")
            adios4dolfinx.write_function(check_file_p, p1, time=t, name="p1")
    ...
    if step==1:
            # I save the first calculated values of Drag and Lift
            DRAG.append(assemble_scalar(drag))
            LIFT.append(assemble_scalar(lift))

    # Write solutions to file (in reality I shouldn't always do this!!!)
    # vtx_u.write(t)
    # vtx_p.write(t)
    ...
    u0.x.array[:] = u1.x.array[:] # Update the value of u0 (which represents u(n-1) in the first step of the IPCS)
